...
let textbox = new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
	el: "#textbox",
	data: {
		items: [],
		copies: []
...
let send = new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
	el: "#send",
	methods: {
		post: () => {
...
function pushJson(data_){
	// jsonを代入
	aryContObj = data_;
	__WEBPACK_IMPORTED_MODULE_2_underscore___default.a.each(data_, json_ => {
		...
		__WEBPACK_IMPORTED_MODULE_2_underscore___default.a.each(json_.contents, ary_ => {
...
function changeJson(data_, cont_) {
	...
	// 全要素のカウント．
	let _num = 0;
	data_ = __WEBPACK_IMPORTED_MODULE_2_underscore___default.a.each(data_, obj_ => {
		if (isDefined(obj_.title)) {
			obj_.title = cont_.items[_num++];
			obj_.sub = cont_.items[_num++];
		}
		obj_.contents = __WEBPACK_IMPORTED_MODULE_2_underscore___default.a.map(obj_.contents, () => {
...
function reqContServ(REQ_, BODY_ = "") {
	return new Promise((resolve_, rej_) => {
		...
		__WEBPACK_IMPORTED_MODULE_1_isomorphic_fetch___default()(REQ_.URL, REQ_.OPTION)
			.then(res_ => {
				if (res_.status >= 400) {
